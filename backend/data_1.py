...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://speedline-frontend.onrender.com"],
...
def load_network_config() -> Dict[str, Any]:
    """Load network configuration from JSON file, with sensible fallback."""
    default_config = {
    ...
    try:
        ...
        config_path = os.path.join(base_dir, "network_config.json")
        if os.path.exists(config_path):
            with open(config_path, "r") as f:
                data = json.load(f)
                stations = data.get("stations")
                sections = data.get("sections")
                if isinstance(stations, list) and isinstance(sections, list):
                    return {"stations": stations, "sections": sections}
        # Try env-provided path
        env_path = os.getenv("NETWORK_CONFIG_PATH")
...
STATIONS = _NETWORK_CONFIG.get("stations", [])
SECTIONS = _NETWORK_CONFIG.get("sections", [])
...
def background_simulation():
    ...
    logger.info("Background train simulation started")
    ...
    while simulation_running:
        try:
            ...
            # Only update if enough time has passed (minimum 1 second between updates)
            if time_since_last_update >= 0.0167:  # ~1 second in minutes
                # Generate section disruptions (less frequently)
                ...
                # Update all trains
                for train_id in list(train_state.trains.keys()):
                    train_state.trains[train_id] = update_train_data(train_state.trains[train_id], time_since_last_update)
    ...
    logger.info("Background train simulation stopped")
...
def generate_section_disruptions():
    ...
    # 5% chance of new disruption per update (reduced from 15%)
...
def calculate_position_progress(train_data: Dict[str, Any], time_elapsed_min: float) -> Dict[str, Any]:
    ...
    if random.random() < 0.01:  # 1% chance per update (reduced from 5%)
...
def update_train_state():
    """Get current train state (now updated by background thread)"""
    global train_state
    ...
    # Background thread handles updates, this just returns current state
    return
...
async def get_network_config():
    """Serve dynamic network configuration (stations, sections, capacities)."""
    try:
        # Reload on each request to pick up changes without restart
        ...
        logger.error(f"Error serving network config: {e}")
        raise HTTPException(status_code=500, detail="Failed to load network config")
