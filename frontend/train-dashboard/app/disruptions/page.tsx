...
export default function DisruptionsPage() {
  ...
  const computeProgress = React.useCallback((startTime: string, endTime: string) => {
    const start = new Date(startTime).getTime();
    const end = new Date(endTime).getTime();
    const now = Date.now();
    const denom = Math.max(end - start, 1);
    const pct = Math.round(((now - start) / denom) * 100);
                      ...
                      <div className="mt-8">
                        <div className="flex justify-between text-sm text-gray-600 mb-3">
                          ...
                          <span className="font-semibold">
                            {mounted ? computeProgress(disruption.start_time, disruption.estimated_end_time) : 0}%
                          </span>
                        </div>
                        <div className="w-full bg-gray-200 rounded-full h-3 shadow-inner">
                          <motion.div 
                            initial={{ width: 0 }}
                            animate={{ 
                              width: `${mounted ? computeProgress(disruption.start_time, disruption.estimated_end_time) : 0}%` 
                            }}
