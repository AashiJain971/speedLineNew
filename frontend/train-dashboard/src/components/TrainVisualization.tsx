...
const SimpleTrainTooltip: React.FC<SimpleTooltipProps> = ({ train, position, isVisible }) => {
  ...
  const getStatusHex = (status: string): string => {
    ...
    switch (key) {
      case 'ontime': return '#10b981'; // green-500
      case 'delayed': return '#ef4444'; // red-500
      case 'arrived': return '#3b82f6'; // blue-500
      case 'cancelled': return '#6b7280'; // gray-500
      case 'waitingtraffic': return '#f59e0b'; // amber-500
      case 'waitingsectiondisrupted': return '#ef4444'; // red-500
      case 'waitingnoroute': return '#8b5cf6'; // violet-500
  ...
  const colorHex = getStatusHex(train.status);
  const tooltipContent = (
    <motion.div
      ...
      className="fixed z-[10000] bg-white rounded-lg shadow-2xl border pointer-events-none"
      style={{
        left: tooltipX,
        top: tooltipY,
        width: tooltipWidth,
    ...
    >
      ...
      <div className={`px-4 py-2 rounded-t-lg text-white`} style={{ backgroundColor: colorHex }}>
      ...
      <div className="p-4 space-y-2">
        ...
        <div className="flex justify-between items-center">
          ...
          <span className={`px-2 py-1 rounded text-xs font-medium text-white`} style={{ backgroundColor: colorHex }}>
      ...
      {/* Tooltip arrow */}
      <div 
        className="absolute w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent"
        style={{
          bottom: '-4px',
    ...
    <>
      <motion.div
        ...
        style={{
          ...
          transition: 'left 0.8s linear, top 0.3s ease, transform 0.2s ease',
          willChange: 'left, top',
        }}
...
const TrainVisualization: React.FC<TrainVisualizationProps> = ({ 
  ...
  useEffect(() => {
    if (snapshotTs) {
      setLastUpdate(snapshotTs);
    } else {
      setLastUpdate(new Date().toISOString());
    }
  }, [trainData, snapshotTs]);
