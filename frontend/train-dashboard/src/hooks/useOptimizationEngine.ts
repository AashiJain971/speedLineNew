...
import { useNetworkConfig, NetworkConfig } from './useNetworkConfig';
...
const useOptimizationEngine = (
  ...
  const sectionCapacity: Record<string, number> = (networkConfig?.sections || []).reduce((acc, s) => {
    acc[s.id] = s.capacity ?? 1;
    return acc;
  }, {} as Record<string, number>);

  const sectionGraph: Record<string, { next: string[]; stations: string[]; length_km: number }> = (() => {
    const sections = networkConfig?.sections || [];
    const byStart: Record<string, string[]> = {};
    sections.forEach((s) => {
      if (!byStart[s.start]) byStart[s.start] = [];
      byStart[s.start].push(s.id);
    });
    const graph: Record<string, { next: string[]; stations: string[]; length_km: number }> = {};
    sections.forEach((s) => {
      const next = byStart[s.end] || []; // sections that start where this one ends
      graph[s.id] = { next, stations: [s.start, s.end], length_km: s.length_km };
    });
    return graph;
  })();
  ...
  const fetchTrainData = useCallback(async (): Promise<TrainData[] | null> => {
    try {
      const response = await axiosInstance.get('/api/train-data', { timeout: 30000 });
...
`;
  ...
  const getOptimizedScheduleWithGroq = useCallback(async (trains: TrainData[], conflicts: Conflict[]): Promise<OptimizationSchedule> => {
    ...
    try {
      ...
      const response = await axiosInstance.post('/api/groq-optimization', {
        prompt,
  ...
  }, [groqApiKey, createOptimizationPrompt]);
  ...
  const saveSchedule = useCallback(async (schedule: OptimizationSchedule): Promise<void> => {
    try {
      ...
      await axiosInstance.post('/api/optimization/results', schedule, {
        timeout: 10000
      });
